        ...
        <>
            <article className='card'>
                <div className='card-flip'>
                     {((spot.bu <= timeInt/1000 && spot.ru <= timeInt/1000) || (spot.ru >= timeInt/1000 && spot.booker.toLowerCase() === accounts[0])) ? <Button size='sm' variant="outline-secondary" onClick={handleShow}>Flip for {(Web3.utils.fromWei(((spot.priceOfSpot)), 'ether'))} <Icon fill="grey" size={12} svg={icon}/></Button> : <Button size='sm' variant="outline-secondary" onClick={handleShow} disabled>Flip</Button>}
                </div>
                <div className='text'>
                    {spot.text}
                </div> 
