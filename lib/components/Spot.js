...
const Spot = ({ spotResPrice, showFlipModal, showSpinnerReset, showSpinnerReserve, showSpinnerFlip, fightData, accounts, spot, onSpotReset, onSpotFlip, onSpotReservation }) => {
        ...
        <>
            <article className='card'>
                ...
                <ul className='card-buttons'>
                  <li>
                      {(spot.flipper.toLowerCase() !=accounts[0] && spot.bu <= timeInt/1000 && spot.ru <= timeInt/1000)  ? <Button style = {{margin: '2px'}} size='sm' variant="outline-secondary" onClick={() => onSpotReservation(spot.SpotNo)}>Reserve for {(Web3.utils.fromWei(((spotResPrice)), 'ether'))}</Button> : <Button style = {{margin: '2px'}} size='sm' variant="outline-secondary" onClick={() => onSpotReservation(spot.SpotNo)} hidden>Reserve for {(Web3.utils.fromWei(((spotResPrice)), 'ether'))}</Button>}
                  </li>
                  <li>
