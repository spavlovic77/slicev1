...
const Spots = ({ fightData, accounts, spots, onSpotReset, onSpotFlip, onSpotReservation, onBuySpot }) => {
    ...
    return (
        ...
        <section className='card-list'>
            {spots.map((spot)=>(
            <Spot fightData={fightData} accounts ={accounts} key={spot.SpotNo} spot={spot} onSpotReservation={onSpotReservation} onSpotFlip={onSpotFlip} onSpotFlip={onSpotFlip} onSpotReset={onSpotReset}    >
            </Spot>    
            )
            )}
        </section>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={()=>onBuySpot(text, link, pic)}>Save Changes</Button>
        </Modal.Footer>
      </Modal>
