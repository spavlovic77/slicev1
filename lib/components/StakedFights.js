...
const StakedFights = ({ stakedFights, onUnstake }) => {
  ...
  if (stakedFights!=undefined) {
    ...
    .map((stakedFights) => [<span></span>, <Link href="/fights/[address]" as={`/fights/${stakedFights.contract}`}>{`Fight `+stakedFights.contract.substring(0,6)+`...`+stakedFights.contract.substring(38,42)}</Link>,
