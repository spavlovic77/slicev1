...
const StakedFights = ({ stakedFights, onUnstake }) => {
  ...
  if (stakedFights!=undefined) {
    ...
    .map((stakedFights, index) => ['', <Link href="/fights/[address]" as={`/fights/${stakedFights.contract}`}>{`Fight `+stakedFights.contract.substring(0,6)+`...`+stakedFights.contract.substring(38,42)}</Link>,
    <span key={index}>{Math.floor(Web3.utils.fromWei(((stakedFights.Staked).toString()), 'ether'))+` SLICE`}</span>,
