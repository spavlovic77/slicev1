...
const handleAddFightData = async () => {
  if (priceInWei==undefined || priceCoef==undefined || extend==undefined || duration==undefined || promo1==undefined || promo2==undefined) {
    ...
    const fight = new web3.eth.Contract(polygonFight.abi, address);
          await fight.methods.addFightData(accounts[0], priceInWei, priceCoef, extend, duration, fightTopic, fightLocation, promo1, promo2, shortLink).send({ from: accounts[0] })
          .on('receipt', receipt => {
...
if (newFights!=undefined) {
    ...
    .map((fight, index) => ['', <span key={index}>{`Fight `+fight.fightContract.substring(0,6)+`...`+fight.fightContract.substring(38,42)}</span>,
    <Button variant='outline-secondary' onClick={()=> handleShow(fight.fightContract)}>Start fight</Button>, ''])
         ...
         {newFights && <Table
            ...
            />}
                ...
                <Form noValidate>
                    ...
                    <FloatingLabel controlId="floatingInput" label="Enter starting price of a spot" className="mb-3">
                    ...
                    <Form.Group className="mb-3" controlId="coef">
                      <Form.Label>By flipping the spot its price will increase by </Form.Label><br></br>
