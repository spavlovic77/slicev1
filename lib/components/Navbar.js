...
import { Select, Icon, Tag, BannerStrip } from 'web3uikit';
...
const notYetMinted = () => {
       ...
       if (isRegistered == 0 && start!='NaN' && mintSpeed!='NaN') {
        const x = new Date();
       ...
       else if (mintedDate!=0 && mintedDate!='undefined' && mintedDate!='NaN' && mintSpeed!='NaN') {
        const x = new Date();
        const time = x.getTime();
        const timeTr=parseInt(time);
       ...
       else if (start!='NaN' && mintSpeed!='NaN') {
        const x = new Date();
        const time = x.getTime();
        const timeTr=parseInt(time);
        const startF=parseInt(start)*1000
        const a = (timeTr - startF) / mintSpeed
        console.log({timeTr},{startF}, {mintSpeed} )
        const b = new BigNumber(a)
        const c = b.dividedBy(1000).toFixed(0)
        const mint = Web3.utils.toBN(c).toString()
        return mint
...
console.log({mintedDate}, {isRegistered}, {start}, {mintSpeed})
    return (
      <>     
              ...
              <ul className='logo'>
               ...
               <ul className='registerOrMint'>
                <li>
                  ...
                  {(!wl && window.location.pathname!='/register') &&<Button size='sm' variant="outline-secondary" onClick={handleRegister}>Register</Button>}
                    ...
                    options={[
                      {
                        ...
                        prefix: <Icon
                        fill={(isRegistered!=0 && wl==false) ? "red" : "green"}
                        size={10}
                        style={{}}
                        svg="user"
                      />
