...
const Map = ({ accounts, slice, fightFactory }) => {
  const router = useRouter()
  const {address} = router.query
  const googlemap = useRef(null);
  // const [addr, setAddress] = useState(undefined)
  // const [listOfStakers, setListOfStakers] = useState(undefined)
  // const [loading, setLoading] = useState(undefined)
  const [loadingMessage, setLoadingMessage] = useState('Loading...');
  const [stakedByFights, setStakedByFight] = useState (undefined)
  ...
  const [mintersData, setList] = useState(undefined)
  const list =[]
  const [pushed, setPushed] = useState(true)

  useEffect(() => {
      const init = async () => {
        if (stakedByFights!=undefined) {
          stakedByFights.map((f) => {
            const mintersData =  slice.methods.getMinter(f.staker).call({ from: accounts[0] }).then(data => {
              list.push({contract: f.staker, Lat: data[3], Lon: data[4], Nick: data[5], Reported: data[1], GoodPoints: data[0]})
              setPushed(false)
              return list;
            })
          });setList(list)
...
const handleMint = async () => {
  ...
  return (
    <>    
      ...
      {loadingMintData && <Navbar staked={staked} vSliceBalance={vSliceBalance} onMint={handleMint} accounts={accounts} slice={slice} fightFactory={fightFactory}/>}
